...
public class EditorDePosesV2 extends JFrame {
    ...
    public EditorDePosesV2() {
        ...
        painelControles.add(new JLabel("--- CABEÇA E ROSTO ORIGINAL ---"));
    ...
    private void atualizarCodigoGerado() {
        String codigo = String.format(
            ...
            "// Rotação Cabeça: %d graus | Offset X: %d | Offset Y: %d\n\n" +
            ...
            rotGlobal, 
            rotCabeca, offCabecaX, offCabecaY,
    ...
    private class CanvasPanel extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            // Instancia o lutador para extrair os métodos oficiais
            IrmaoMiau miauInstancia = isBranco ? new IrmaoMiauBranco(x, y, s) : new IrmaoMiauPreto(x, y, s);

            // --- DESENHO DO ROSTO E CABEÇA ORIGINAL ---
            Graphics2D gCabeca = (Graphics2D) g2d.create();
            
            // Aplica os offsets dos sliders da cabeça
            gCabeca.translate(offCabecaX * s, offCabecaY * s);
            
            // Aplica a rotação da cabeça (pivot aproximado no centro do rosto/pescoço)
            double pivotCabecaX = x + 35 * s;
            double pivotCabecaY = y + 53 * s; 
            gCabeca.rotate(Math.toRadians(rotCabeca), pivotCabecaX, pivotCabecaY);
            
            // Truque de Mestre: Reflection para puxar a sua cabecinha original!
            try {
                java.lang.reflect.Method metodoCabeca = null;
                Class<?> clazz = miauInstancia.getClass();
                // Sobe na hierarquia até achar o método desenharCabeca da classe mãe
                while (clazz != null && metodoCabeca == null) {
                    try {
                        metodoCabeca = clazz.getDeclaredMethod("desenharCabeca", Graphics2D.class);
                ...
                if (metodoCabeca != null) {
                    ...
                    // Aqui a mágica acontece: ele desenha a exata cabeça que você programou na sua engine!
                    metodoCabeca.invoke(miauInstancia, gCabeca); 
                }
            } catch (Exception ex) {
                System.err.println("Aviso: Método desenharCabeca não encontrado. Certifique-se de que ele existe na classe IrmaoMiau.");
            }
            gCabeca.dispose();
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new EditorDePosesV2().setVisible(true));
    }
}